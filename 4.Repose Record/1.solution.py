...
for guard, shifts in records._shifts.items():
    ...
    awake = 0
    for shift in shifts:
        for entry in reversed(shift):
            if awake:
                minutes_for_guard += (awake - entry[1])
                awake = 0
            else:
                awake = entry[1]
    asleep_minutes[guard] = minutes_for_guard

guard = max(asleep_minutes, key=lambda x: asleep_minutes[x])
guard_shifts = records._shifts[guard]

minutes = [0 for i in range(60)]
for shift in records._shifts[guard]:
    print(shift)
    start = 0
    state = 0
    try:
...
minutes = [(i, minutes[i]) for i in range(60)]
minute = max(minutes, key=lambda x:x[1])[0]
